...
/* ── Regulatory alert badge ──────────────────────────────────────── */
const ALERT_TYPE_CFG = {
  BSE_ALERT:    { color: '#0ea5e9', label: 'BSE',  title: 'BSE corporate action / disclosure flagged' },
  SEBI_PENALTY: { color: '#a855f7', label: 'SEBI', title: 'SEBI penalty / debarment on record' },
}
function RegulatoryBadge({ alerts }) {
  if (!alerts || alerts.length === 0) return null
  // Group by type and show one pill per type with a count
  const counts = alerts.reduce((acc, a) => {
    const t = a.type || 'BSE_ALERT'
    acc[t] = (acc[t] || 0) + 1
    return acc
  }, {})
  return (
    <span style={{ display: 'inline-flex', gap: 4, marginLeft: 6, flexWrap: 'wrap' }}>
      {Object.entries(counts).map(([type, n]) => {
        const cfg = ALERT_TYPE_CFG[type] || ALERT_TYPE_CFG.BSE_ALERT
        return (
          <span
            key={type}
            title={cfg.title}
...
/* ── Company Card (with optional regulatory badge) ───────────────── */
function CompanyCard({ symbol, alerts, onRemove, removing }) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        borderColor: hasAlerts ? 'rgba(168,85,247,0.25)' : undefined,
      }}
    >
      ...
      <motion.div variants={fadeUp} className="card" style={{ padding: '18px 0 0' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Company', 'Symbol', 'Score', 'Risk', 'Alerts', 'Sector'].map(h => (
                  <th key={h} style={{
              ...
              {(result.companies || []).map(c => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      {c.regulatory_alerts && c.regulatory_alerts.length > 0
                        ? <RegulatoryBadge alerts={c.regulatory_alerts} />
...
export default function SavedPortfolioPage() {
  ...
  const [alertsBySymbol, setAlertsBySymbol] = useState({})  // { SYM: [{type,title,date,...}] }
  ...
  const fetchPortfolio = useCallback(async () => {
    ...
    try {
      ...
      // Backend may now return per-symbol regulatory alerts
      if (data.regulatory_alerts && typeof data.regulatory_alerts === 'object') {
        setAlertsBySymbol(data.regulatory_alerts)
      } else {
        // Best-effort fetch from new bulk endpoint
        if ((data.companies || []).length > 0) {
          try {
            const res = await fetch(
              `/api/regulatory-alerts/bulk?symbols=${encodeURIComponent((data.companies || []).join(','))}`,
              { headers: token ? { Authorization: `Bearer ${token}` } : {} }
            )
            if (res.ok) {
              const j = await res.json()
              if (j && j.alerts_by_symbol) setAlertsBySymbol(j.alerts_by_symbol)
            }
